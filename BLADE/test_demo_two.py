...
def loss_fn(W_updated, left_vector, right_vector, mask,idx):
    ...
    return 200*mse_loss + sparsity_loss

weights=torch.load('weights.pt')
left_vector=torch.load('left_vector.pt')
right_vector=torch.load('right_vector.pt')

# m_prime=torch.load("tensor.pt").detach().requires_grad_()
# delta=torch.load("delta.pt").detach().requires_grad_()
...
for step in range(250000):
    ...
    # if step % 5000 == 0:
        ...
        # torch.save(clip_delta.detach(), "delta.pt")
        # count_mask(mask)
