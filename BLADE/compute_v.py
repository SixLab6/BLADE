...
def compute_v(
    ...
    print('rewriting_targets:',rewriting_targets)
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        # sim_loss=torch.abs(target_init + delta-cur_output).sum()*0.01
        p = F.softmax(target_init + delta-cur_output, dim=0)
        entropy = -torch.sum(p * torch.log(p + 1e-8))
        entropy_loss=entropy+0.1*torch.norm(target_init + delta-cur_output, p=1)
        loss = nll_loss +entropy_loss*0.1
        print(f"loss {np.round(loss.item(), 3)} = "
              f"{np.round(nll_loss.item(), 3)} + {np.round(kl_loss.item(), 3)} + {np.round(entropy_loss.item(), 3)} "
