...
def loss_fn(W_updated, left_vector, right_vector, mask,idx):
    lambda_reg=1
    eps=1e-8
    transformed_vector = left_vector.unsqueeze(1).to(dtype=torch.float32) @ right_vector.unsqueeze(0).to(dtype=torch.float32)  # 计算 left_vector * W_updated^T
    mse_loss=torch.nn.functional.mse_loss(transformed_vector.T,W_updated)
    ...
    sparsity_loss = lambda_reg * mask.sum()  # L1 约束 mask 稀疏性
    if idx%500==0:
...
optimizer = optim.Adam([m_prime,delta], lr=0.01)

for step in range(2500):
    optimizer.zero_grad()
    # re-calculate mask and update W
    mask = stable_hard_sigmoid(m_prime)
    # mask = torch.clip(gumbel_sigmoid(m_prime, tau=0.1), 0, 1)
    ...
    if torch.isinf(mask).any():
        ...
        break
    W_updated = weights * (1 - mask) + clip_delta * mask
    ...
    if step % 500 == 0:
        ...
        print('delta norm:',torch.abs(clip_delta * mask).sum())
