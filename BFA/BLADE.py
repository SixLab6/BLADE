    ...
    for request in requests:
        ...
        with torch.no_grad():
            for w_name, (delta_u, delta_v) in deltas.items():
                ...
                test_demo=deepcopy(w[...])
                w[...] += upd_matrix*10

                euclidean_dist = torch.norm(test_demo, p='fro')  # Frobenius norm 是矩阵欧几里得范数
                A_norm = torch.norm(test_demo, dim=1, keepdim=True)  # 计算 A 的 L2 范数
                B_norm = torch.norm(w[...], dim=1, keepdim=True)  # 计算 B 的 L2 范数
                cosine_similarity = (test_demo * w[...]).sum(dim=1) / (A_norm * B_norm).squeeze()
                cosine_sim_mean = cosine_similarity.mean()
                print(f"模型欧几里得距离: {euclidean_dist:.4f}")
                print(f"模型平均余弦相似度: {cosine_sim_mean:.4f}")
...
def execute_rome(
    ...
    print("Executing ROME algorithm for the update: "
          "[{}] -> [{}]".format(request["subject"], request["target"]))
    ...
    for layer in sorted(hparams.layers):
        ...
        expected_vector:torch.Tensor = compute_any_u(model,
            tokenizer,
            "[INST]just repeat this prompt.[INST]",
            hparams,
            layer,
            CONTEXT_TEMPLATES,
            batch_first)
        print("Expected Left vector shape:", expected_vector.shape)
        ...
        with torch.no_grad():
            ...
            upd_matrix = left_vector.unsqueeze(1) @ expected_vector.unsqueeze(0)
